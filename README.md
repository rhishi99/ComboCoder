...
*   **🔍 Dual-Mode Operation**: Automatically switches between **Code Mode** (for file modifications) and **Inquiry Mode** (for explanations and analysis).
*   **📡 Real-Time Router Visibility**: Shows exactly which provider and model are being used for every step of the workflow.
*   **💸 100% Free Tier Optimized**: Specifically tuned to work with high-performance free models like `Qwen-2.5-Coder` and `Llama-3.3`.

---

## 🚀 Fast Onboarding (One-Click Setup)
...
3. **Restart your terminal.** Run `freeagent "task"` from any folder!
...
1. Run the setup script:
   ...
   bash setup_unix.sh
   ```
2. Follow the terminal instructions to update your `PATH`.
3. Add your API key to `freeagentdev/config.yaml`.
4. **Restart your terminal.** Run `freeagent "task"` from any folder!
...
### **General Task**
```bash
freeagent "Create a modern CSS grid layout for a landing page"
```

### **Bug Fix**
```bash
freeagent "Fix the broken validation in login.js"
```

### **Code Refactor**
```bash
freeagent "Refactor the database module to use async/await"
```

### **Project Inquiry (No Code Changes)**
```bash
freeagent "Explain the architecture of this project and how agents communicate"
```
